...
void ast_free(AST_T* ast)
{
    ...
    /*if (ast->scope)
    {
        ys_scope_T* scope = (ys_scope_T*) ast->scope;
...
static AST_T* ast_r_scope(AST_T* ast, struct ys_scope_T* scope)
{
    ast->scope = scope;
    return ast;
}
...
char* ast_to_string(AST_T* ast)
{
    switch (ast->type)
    {
        ...
        case AST_CHAR: return ys_char_to_string(ast->char_value); break;
...
char* ast_list_access_to_string(AST_T* ast)
{
    ...
    return ys_init_str("[ list_access ]");
}
